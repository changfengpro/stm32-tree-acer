...
CANInstance *CANRegister(CAN_Init_Config_s *config)
{
    if (!idx)
    {
        ...
        //[bsp_can] CAN Service Init");
    }
    if (idx >= CAN_MX_REGISTER_CNT) // 超过最大实例数
    {
        while (1);
            //[bsp_can] CAN instance exceeded MAX num, consider balance the load of CAN bus");
    }
    ...
    { // 重复注册 | id重复
        ...
        {
            while (1);
                //[}bsp_can] CAN id crash ,tx [%d] or rx [%d] already registered", &config->tx_id, &config->rx_id);
        }
    }
...
uint8_t CANTransmit(CANInstance *_instance, float timeout)
{
    ...
    while (HAL_CAN_GetTxMailboxesFreeLevel(_instance->can_handle) == 0) // 等待邮箱空闲
    {
        ...
        {
            //[bsp_can] CAN MAILbox full! failed to add msg to mailbox. Cnt [%d]", busy_count);
            busy_count++;
            return 0;
        }
    }
...
void CANSetDLC(CANInstance *_instance, uint8_t length)
{
    ...
    if (length > 8 || length == 0) // 安全检查
        while (1)
            //[bsp_can] CAN DLC error! check your code or wild pointer");
