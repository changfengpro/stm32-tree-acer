...
extern slid_avg_filter_t Slid_filter[5];	//初始化滤波器示例
extern float Velocity[5];
extern float Last_Velocity[5];
float Velocity_Filter[5];
uint32_t count[3] = {0,0,0,0};
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if(index > 0 && index < 5)
  {
    ...
    Velocity_Filter[index] = Slid_avg_filter(&Slid_filter[index], Velocity[index]);
