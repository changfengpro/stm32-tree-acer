...
void ChassisTask()
{
    ...
    pid_ref[1] = ver_pid_calc(&pid[1], vt_rf, Velocity_Filter[1]);
    pid_ref[2] = ver_pid_calc(&pid[2], vt_lf, Velocity_Filter[2]);
    pid_ref[3] = ver_pid_calc(&pid[3], vt_lb, Velocity_Filter[3]);
    pid_ref[4] = ver_pid_calc(&pid[4], vt_rb, Velocity_Filter[4]);

    set_motor_value_CAN1(0x200, 0, 0, pid_ref[3], pid_ref[4]);
    set_motor_value_CAN2(0x200, pid_ref[1], pid_ref[2],0,0);
