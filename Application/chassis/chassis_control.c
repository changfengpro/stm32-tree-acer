...
void ChassisTask()
{
    ...
    pid_ref[1] = ver_pid_calc(&pid[1], vt_rf, motor_info[1].rotor_speed);
    pid_ref[2] = ver_pid_calc(&pid[2], vt_lf, motor_info[2].rotor_speed);
    pid_ref[3] = ver_pid_calc(&pid[3], vt_lb, motor_info[3].rotor_speed);
    pid_ref[4] = ver_pid_calc(&pid[4], vt_rb, motor_info[4].rotor_speed);

    set_motor_value(0x200, pid_ref[1], pid_ref[2], pid_ref[3], pid_ref[4]);
